...
#' 
stan_jm.fit <- function(y_mod_stuff = NULL, e_mod_stuff = NULL, a_mod_stuff = NULL,
                        assoc = NULL, time_var, id_var, family = gaussian,
                        basehaz = NULL, quadnodes = 15, 
                        init = "model_based", offset = NULL,
                        y_weights = NULL, e_weights = NULL, ...,				          
  ...
  M <- if (!is.null(y_mod_stuff)) length(y_mod_stuff) else 1L # dud entry of 1L 
  
  # Number of association parameters
  a_K <- if (!is.null(a_mod_stuff)) attr(a_mod_stuff, "K") else NULL
  
  # Check family and link (also checked in stan_jm)
  ...
  if (!is.null(y_mod_stuff)) {
    ...
    y_prior_stuff <- 
      Map(autoscale_prior, y_prior_stuff, y_mod_stuff, QR = QR, use_x = TRUE)
    y_prior_intercept_stuff <- 
      Map(autoscale_prior, y_prior_intercept_stuff, y_mod_stuff, QR = QR, use_x = FALSE)
    y_prior_aux_stuff <- 
      Map(autoscale_prior, y_prior_aux_stuff, y_mod_stuff, QR = QR, use_x = FALSE)
  } else y_prior_stuff <- y_prior_intercept_stuff <- y_prior_aux_stuff <- NULL
  
  # Priors for event submodel
  if (!is.null(e_mod_stuff)) {
    e_prior_stuff <- 
      handle_glm_prior(
        priorEvent,
        nvars = e_mod_stuff$K,
    ...
    e_prior_stuff <- 
      autoscale_prior(e_prior_stuff, e_mod_stuff, QR = QR, use_x = TRUE)
    e_prior_intercept_stuff <- 
      autoscale_prior(e_prior_intercept_stuff, e_mod_stuff, QR = QR, use_x = FALSE)
    e_prior_aux_stuff <- 
      autoscale_prior(e_prior_aux_stuff, e_mod_stuff, QR = QR, use_x = FALSE)
  } else e_prior_stuff <- e_prior_intercept_stuff <- e_prior_aux_stuff <- NULL
  
  # Priors for association parameters
  if (!is.null(a_mod_stuff)) {
    a_prior_stuff <- 
      handle_glm_prior(
        priorAssoc,
        nvars = a_K,
    ...
    a_prior_stuff <- 
      autoscale_prior(a_prior_stuff, a_mod_stuff, QR = QR, use_x = FALSE, assoc = assoc)
  } else a_prior_stuff <- NULL
  
  #-------------------------
  # Data for export to Stan
  #-------------------------
  
  standata <- list(prior_PD = as.integer(prior_PD))
  
  #----- Longitudinal submodels
  
  if (!is.null(y_mod_stuff)) {
    # Priors
    standata$prior_dist                <- fetch_array(y_prior_stuff, "prior_dist")
    standata$prior_dist_for_intercept  <- fetch_array(y_prior_intercept_stuff, "prior_dist")  
    standata$prior_dist_for_aux        <- fetch_array(y_prior_aux_stuff, "prior_dist")
    standata$prior_mean                <- fetch_array(y_prior_stuff,           "prior_mean")
    standata$prior_scale               <- fetch_array(y_prior_stuff,           "prior_scale") 
    standata$prior_df                  <- fetch_array(y_prior_stuff,           "prior_df")
    standata$prior_mean_for_intercept  <- fetch_array(y_prior_intercept_stuff, "prior_mean")
    standata$prior_scale_for_intercept <- fetch_array(y_prior_intercept_stuff, "prior_scale") 
    standata$prior_df_for_intercept    <- fetch_array(y_prior_intercept_stuff, "prior_df")
    standata$prior_mean_for_aux        <- fetch_array(y_prior_aux_stuff,       "prior_mean")
    standata$prior_scale_for_aux       <- fetch_array(y_prior_aux_stuff,       "prior_scale")
    standata$prior_df_for_aux          <- fetch_array(y_prior_aux_stuff,       "prior_df")
    standata$global_prior_scale        <- fetch_array(y_prior_stuff, "global_prior_scale")
    standata$global_prior_df           <- fetch_array(y_prior_stuff, "global_prior_df")
    ...
    standata$has_weights       <- as.integer(!is.null(y_weights))
    ...
    standata$weights <- 
      if (!is.null(y_weights)) as.array(unlist(y_weights)) else as.array(numeric(0))
    standata$offset  <- 
    ...
    standata$long_lp  <- as.integer(long_lp)
    ...
    cnms <- get_common_cnms(fetch(y_mod_stuff, "cnms")) # single cnms for all submodels
    cnms_nms <- names(cnms)
  ...
  if (!is.null(e_mod_stuff)) {
    ...
    standata$event_lp <- as.integer(event_lp)
    ...
    standata$e_has_intercept <- as.integer(e_mod_stuff$has_intercept)
  ...
  if (!is.null(a_mod_stuff)) {
    ...
    # Indicator for any association structure
    standata$assoc <- as.integer(a_K > 0L) # 1 = yes
    ...
    auc_quadnodes   <- attr(a_mod_stuff, "auc_quadnodes")
    auc_quadweights <- attr(a_mod_stuff, "auc_quadweights")
  ...
  if ((!is.null(y_mod_stuff)) && (!is.null(e_mod_stuff))) {
    stanfit <- stanmodels$jm
  } else if ((!is.null(y_mod_stuff)) && is.null(e_mod_stuff)) {
    stanfit <- stanmodels$mvmer
  } else if (is.null(y_mod_stuff) && (!is.null(e_mod_stuff))) {
    stanfit <- stanmodels$survtd
  } else stop("Bug found: y_mod_stuff and e_mod_stuff are both NULL.")
  ...
  e_int_nms <- if (e_mod_stuff$has_intercept) "Event|(Intercept)"
  ...
  # temporary summary of priors 
  # -- in future will use adapted version of summarize_glm_prior
  ...
  # n_grps is num. of levels within each grouping factor
  if (!is.null(standata$l)) {
    n_grps <- standata$l - 1
    names(n_grps) <- cnms_nms  
  } else n_grps <- NULL
  
  return(structure(stanfit, prior.info = prior.info, n_grps = n_grps))
}
...
# @return A named list
handle_glmod <- function(mc, family, sparse) {
